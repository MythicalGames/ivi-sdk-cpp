...
#define IVI_EXIT_FAILURE()
    ...
    namespace ivi
    {
        enum class LogLevel : int
        {
            ...
            INFO        = 1 << 3,
            VERBOSE     = 1 << 4,
            NTRACE      = 1 << 5,   // network tracing
            DTRACE      = 1 << 6    // debug tracing
        };

        using LogFunc           = void(*)(LogLevel, const std::string&);
        using LogStreamFunc     = void(*)(LogLevel, std::ostringstream& oss);
        ...
        void IVILog(const LogLevel logLevel, Args&&... args)
        {
            ...
            auto _(std::initializer_list<int> { ((oss << std::forward<Args>(args)), 0)... });
